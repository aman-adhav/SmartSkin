...
A safer alternative to Smart Skin : The design and fabrication of
Piezoresistive Trackpad to simulate “touch” for bionic limbs and robots

A Science Fair Project Presented to

Bay Area Science and Engineering Fair 2017
...
Appendix A : Arduino four corner sensing code {#h.xr3v4os4bxy5 .c12}
=============================================
...
  int c = (sensorValue1 \* sensorValue1)- (sensorValue2 \*
sensorValue2)-(sidexone\*sidexone)+(sidextwo\*sidextwo)-(sideyone\*sideyone)+(sideytwo\*sideytwo);

  int d  = (-1\*2\*sidextwo)+(2\*sidexthree);

  int e = ((-1\*2\*sideytwo)+2\*sideythree);

  int f = (sensorValue2 \* sensorValue2)- (sensorValue3 \*
sensorValue3)-(sidextwo\*sidextwo)+(sidexthree\*sidexthree)-(sideytwo\*sideytwo)+(sideythree\*sideythree);

  int g  = (−2\*x2+2\*x4);

  int h = (−2\*sideytwo+2\*y4);

  int i = (sensorValue2 \* sensorValue2)- (sensorValue4 \*
sensorValue4)-(x2\*x2)+(x4\*x4)-(y4\*y4)+(y4\*y4);
...
Appendix B : Arduino X, Y coordinate determination program {#h.fqsf4njyitfu .c12}
==========================================================
...
Appendix C : Processing circular intersection visualization {#h.xci3z3ikphjx .c12}
===========================================================
...
  myPort.bufferUntil('\\n');  // don't generate a serialEvent() until
you get a newline (\\n) byte
...
Appendix D : Processing trilateration X,Y point visualization {#h.jvcp4wlz8hvj .c12}
=============================================================
...
    PVector s = PVector.sub(s2.p, s1.p).div(PVector.sub(s2.p,
s1.p).mag());
...
    PVector t = PVector.sub(s3.p, s1.p).sub(PVector.mult(s,
a)).div(PVector.sub(s3.p, s1.p).sub(PVector.mult(s, a)).mag());
...
    float y = ((sq(s1.d) - sq(s3.d) + sq(a) + sq(b)) / (b \* 2)) - ((a /
b) \* x);
...
    public float d; // distance from sensor to target (radius of the
circle)
...
  ellipse(height,0, scaleValue\*sensorValues[0],
scaleValue\*sensorValues[0]);

  ellipse(0,width, scaleValue\*sensorValues[1],
scaleValue\*sensorValues[1]);

  ellipse(height,width, scaleValue\*sensorValues[2],
scaleValue\*sensorValues[2]);

  ellipse(0,0, scaleValue\*sensorValues[3],
scaleValue\*sensorValues[3]);
...
  sensorValueX =
((sensorValues[2]\*sensorValues[1])-(sensorValues[1]\*sensorValues[1])+600\*600)/2000;
...
  sensorValueX =
((sensorValues[3]\*sensorValues[3])-(sensorValues[2]\*sensorValues[2])+600\*600)/2000;

  sensorValueX1 =
((sensorValues[0]\*sensorValues[0])-(sensorValues[1]\*sensorValues[1])+600\*600)/2000;

  sensorValueY =
((sensorValues[3]\*sensorValues[3])-(sensorValues[2]\*sensorValues[2])+(600\*600))/2000;

  sensorValueY1 =
((sensorValues[1]\*sensorValues[1])-(sensorValues[0]\*sensorValues[0])+(600\*600))/2000;
...
![](images/image26.png)![](images/image10.png)
